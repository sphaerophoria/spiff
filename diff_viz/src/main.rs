use eframe::{
    egui::{self, Color32, Galley, Painter, Pos2, TextStyle, Ui},
    epaint::Stroke,
};
...
fn main() {
    ...
    eframe::run_native(
        "My egui App",
        native_options,
        Box::new(|cc| Box::new(MyEguiApp::new(cc, args))),
    )
    .unwrap();
}
...
impl eframe::App for MyEguiApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            let a_laid_out = lay_out_elems(self.a.as_ref(), ui);
            let b_laid_out = lay_out_elems(self.b.as_ref(), ui);
            ...
            for y_idx in 0..=self.b.len() {
                for x_idx in 0..=self.a.len() {
                    ...
                    if y_idx < self.b.len()
                        && x_idx < self.a.len()
                        && self.b.get(y_idx) == self.a.get(x_idx)
                    {
